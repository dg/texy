...
- **`parseLine(Texy $texy, string $s): LineParser`** – creates a `LineParser` with this element as the container and parses the string's inline syntaxes; results become the element's children. Returns the parser (some callers inspect it afterwards).

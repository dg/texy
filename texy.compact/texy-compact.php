 ...
 * @package    Texy
 * @category   Text
 * @version    2.0beta for PHP5 $Revision: 72 $ $Date: 2007-03-01 05:09:33 +0100 (čt, 01 III 2007) $
 */if(version_compare(PHP_VERSION,'5.0.0','<'))die('Texy! needs PHP version 5');define('TEXY','Version 2.0beta $Revision: 72 $');define('TEXY_DIR',dirname(__FILE__).'/');

define('TEXY_CHAR','A-Za-z\x{c0}-\x{02af}\x{0370}-\x{1eff}');define('TEXY_MARK',"\x01-\x04\x14-\x1F");define('TEXY_MARK_SPACES',"\x01-\x04");define('TEXY_MARK_N',"\x14\x18-\x1F");define('TEXY_MARK_I',"\x15\x18-\x1F");define('TEXY_MARK_T',"\x16\x18-\x1F");define('TEXY_MARK_B',"\x17\x18-\x1F");define('TEXY_MODIFIER','(?:\ *(?<= |^)\.(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\})(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\})??(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\})??)');define('TEXY_MODIFIER_H','(?:\ *(?<= |^)\.(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\}|(?:<>|>|=|<))(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\}|(?:<>|>|=|<))??(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\}|(?:<>|>|=|<))??(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\}|(?:<>|>|=|<))??)');define('TEXY_MODIFIER_HV','(?:\ *(?<= |^)\.(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\}|(?:<>|>|=|<)|(?:\^|\-|\_))(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\}|(?:<>|>|=|<)|(?:\^|\-|\_))??(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\}|(?:<>|>|=|<)|(?:\^|\-|\_))??(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\}|(?:<>|>|=|<)|(?:\^|\-|\_))??(\([^\n\)]+\)|\[[^\n\]]+\]|\{[^\n\}]+\}|(?:<>|>|=|<)|(?:\^|\-|\_))??)');define('TEXY_IMAGE','\[\*([^\n'.TEXY_MARK.']+)'.TEXY_MODIFIER.'? *(\*|>|<)\]');define('TEXY_LINK_REF','\[[^\[\]\*\n'.TEXY_MARK.']+\]');define('TEXY_LINK_URL','(?:\[[^\]\n]+\]|(?!\[)[^\s'.TEXY_MARK.']*?[^:);,.!?\s'.TEXY_MARK.'])');define('TEXY_LINK','(?::('.TEXY_LINK_URL.'))');define('TEXY_LINK_N','(?::('.TEXY_LINK_URL.'|:))');define('TEXY_EMAIL','[a-z0-9.+_-]+@[a-z0-9.+_-]{2,}\.[a-z]{2,}'); 
...
extends
TexyModule{const
DYNAMIC=1,FIXED=2;protected$default=array('headingSurrounded'=>TRUE,'headingUnderlined'=>TRUE);public$top=1;public$title;public$balancing=TexyHeadingModule::DYNAMIC;public$levels=array('#'=>0,'*'=>1,'='=>2,'-'=>3,);private$_rangeUnderline;private$_deltaUnderline;private$_rangeSurround;private$_deltaSurround;public
function
...
TexyModifier;$mod->setProperties($mMod1,$mMod2,$mMod3,$mMod4);$mod->decorate($this->texy,$el);$level=$this->levels[$mLine];$el->eXtra['level']=$level;$el->eXtra['top']=$this->top;$el->eXtra['deltaLevel']=0;if($this->balancing===self::DYNAMIC)$el->eXtra['deltaLevel']=&$this->_deltaUnderline;$el->parseLine($this->texy,trim($mContent));$parser->children[]=$el;if($this->title===NULL)$this->title=Texy::wash($el->getContent());$this->_rangeUnderline[0]=min($this->_rangeUnderline[0],$level);$this->_rangeUnderline[1]=max($this->_rangeUnderline[1],$level);$this->_deltaUnderline=-$this->_rangeUnderline[0];$this->_deltaSurround=-$this->_rangeSurround[0]+($this->_rangeUnderline[1]?($this->_rangeUnderline[1]-$this->_rangeUnderline[0]+1):0);}public
function
processBlockSurround($parser,$matches){list(,$mLine,,$mContent,$mMod1,$mMod2,$mMod3,$mMod4)=$matches;$el=new
TexyHeadingElement;$mod=new
TexyModifier;$mod->setProperties($mMod1,$mMod2,$mMod3,$mMod4);$mod->decorate($this->texy,$el);$level=7-min(7,max(2,strlen($mLine)));$el->eXtra['level']=$level;$el->eXtra['top']=$this->top;$el->eXtra['deltaLevel']=0;if($this->balancing===self::DYNAMIC)$el->eXtra['deltaLevel']=&$this->_deltaSurround;$el->parseLine($this->texy,trim($mContent));$parser->children[]=$el;if($this->title===NULL)$this->title=Texy::wash($el->getContent());$this->_rangeSurround[0]=min($this->_rangeSurround[0],$level);$this->_rangeSurround[1]=max($this->_rangeSurround[1],$level);$this->_deltaSurround=-$this->_rangeSurround[0]+($this->_rangeUnderline[1]?($this->_rangeUnderline[1]-$this->_rangeUnderline[0]+1):0);}}class
TexyHeadingElement
extends
TexyHtml{public
function
startTag(){$level=$this->eXtra['level']+$this->eXtra['deltaLevel']+$this->eXtra['top'];$this->elName='h'.min(6,max(1,$level));return
parent::startTag();}} 
...
TexyFigureModule
extends
TexyModule{protected$default=array('figure'=>TRUE);public$class='figure';public$leftClass='figure-left';public$rightClass='figure-right';public
function
...
function
processBlock($parser,$matches){list(,$mMod1,$mMod2,$mMod3,$mMod4,$mBullet,$mNewLine)=$matches;$tx=$this->texy;$bullet='';foreach($this->translate
as$type)if(preg_match('#'.$type[0].'#Au',$mBullet)){$bullet=$type[0];$tag=$type[2];$style=$type[1];break;}$el=TexyHtml::el($tag);$el->style['list-style-type']=$style;$mod=new
...
extends
TexyListModule{protected$default=array('listDefinition'=>TRUE);public$bullets=array('*'=>TRUE,'-'=>TRUE,'+'=>TRUE,);private$translate=array('*'=>array('\*'),'-'=>array('[\x{2013}-]'),'+'=>array('\+'),);public
function
init(){$bullets=array();foreach($this->bullets
as$bullet=>$allowed)if($allowed)$bullets[]=$this->translate[$bullet][0];$this->texy->registerBlockPattern(array($this,'processBlock'),'#^(?:'.TEXY_MODIFIER_H.'\n)?'.'(\S.*)\:\ *'.TEXY_MODIFIER_H.'?\n'.'(\ +)('.implode('|',$bullets).')\ +\S.*$#mUu','listDefinition');}public
function
processBlock($parser,$matches){list(,$mMod1,$mMod2,$mMod3,$mMod4,$mContentTerm,$mModTerm1,$mModTerm2,$mModTerm3,$mModTerm4,$mSpaces,$mBullet)=$matches;$tx=$this->texy;$bullet='';foreach($this->translate
as$type)if(preg_match('#'.$type[0].'#Au',$mBullet)){$bullet=$type[0];break;}$el=TexyHtml::el('dl');$mod=new
...
preg_replace_callback('#[^\ \n\t\x15\x16\x17'.TEXY_MARK_SPACES.'\x{2013}\x{2014}\x{ad}-]{'.$this->wordLimit.',}#u',array($this,'_replace'),$text);}private
function
...
TexyModifier;$mod->setProperties($mMod1,$mMod2,$mMod3);if($tag==='acronym'||$tag==='abbr'){$mod->title=trim(Texy::decode($mLink));$mLink=NULL;}$el=TexyHtml::el($tag);$mod->decorate($tx,$el);if($mLink&&$tag==='q'){$el->cite=$tx->quoteModule->citeLink($mLink);$mLink=NULL;}$el->setContent($content);if(is_callable(array($tx->handler,'phrase')))$tx->handler->phrase($tx,$name,$mod,$mLink,$el);$content=$el->export($tx);if($mLink){$req=$tx->linkModule->parse($mLink,$mMod1,$mMod2,$mMod3,$mContent);$el=$tx->linkModule->factory($req)->setContent($content);if(is_callable(array($tx->handler,'link')))$tx->handler->link($tx,$req,$el);$content=$el->export($tx);}$parser->again=TRUE;return$content;}public
function
...
function
init(){$pairs=array('#(?<!"|\w)"(?!\ |")(.+)(?<!\ |")"(?!")()#U'=>$this->doubleQuotes[0].'$1'.$this->doubleQuotes[1],'#(?<!\'|\w)\'(?!\ |\')(.+)(?<!\ |\')\'(?!\')()#Uu'=>$this->singleQuotes[0].'$1'.$this->singleQuotes[1],'#(?<![.\x{2026}])\.{3,4}(?![.\x{2026}])#mu'=>"\xe2\x80\xa6",'#(?<=[\d ])-(?=[\d ])#'=>"\xe2\x80\x93",'#,-#'=>",\xe2\x80\x93",'#(?<!\d)(\d{1,2}\.) (\d{1,2}\.) (\d\d)#'=>"\$1\xc2\xa0\$2\xc2\xa0\$3",'#(?<!\d)(\d{1,2}\.) (\d{1,2}\.)#'=>"\$1\xc2\xa0\$2",'#([\x{2013}\x{2014}]) #u'=>"\$1\xc2\xa0",'# --- #'=>" \xe2\x80\x94\xc2\xa0",'# -- #'=>" \xe2\x80\x93\xc2\xa0",'# -> #'=>" \xe2\x86\x92 ",'# <- #'=>" \xe2\x86\x90 ",'# <-> #'=>" \xe2\x86\x94 ",'#(\d+)( ?)x\\2(\d+)\\2x\\2(\d+)#'=>"\$1\xc3\x97\$3\xc3\x97\$4",'#(\d+)( ?)x\\2(\d+)#'=>"\$1\xc3\x97\$3",'#(?<=\d)x(?= |,|.|$)#m'=>"\xc3\x97",'#(\S ?)\(TM\)#i'=>"\$1\xe2\x84\xa2",'#(\S ?)\(R\)#i'=>"\$1\xc2\xae",'#\(C\)( ?\S)#i'=>"\xc2\xa9\$1",'#(\d{1,3}) (\d{3}) (\d{3}) (\d{3})#'=>"\$1\xc2\xa0\$2\xc2\xa0\$3\xc2\xa0\$4",'#(\d{1,3}) (\d{3}) (\d{3})#'=>"\$1\xc2\xa0\$2\xc2\xa0\$3",'#(\d{1,3}) (\d{3})#'=>"\$1\xc2\xa0\$2",'#(\S{100,}) (\S+)$#'=>"\$1\xc2\xa0\$2",'#(?<=^| |\.|,|-|\+|\x15)(['.TEXY_MARK_N.']*\d+['.TEXY_MARK_N.']*) (['.TEXY_MARK_N.']*['.TEXY_CHAR.'\x{b0}-\x{be}\x{2020}-\x{214f}])#mu'=>"\$1\xc2\xa0\$2",'#(?<=^|[^0-9'.TEXY_CHAR.'])(['.TEXY_MARK_N.']*[ksvzouiKSVZOUIA]['.TEXY_MARK_N.']*) (['.TEXY_MARK_N.']*[0-9'.TEXY_CHAR.'])#mu'=>"\$1\xc2\xa0\$2",);$this->pattern=array_keys($pairs);$this->replace=array_values($pairs);}public
function

 ...
 * @package    Texy
 * @category   Text
 * @version    2.0beta for PHP5 $Revision: 75 $ $Date: 2007-03-01 22:39:55 +0100 (čt, 01 III 2007) $
 */if(version_compare(PHP_VERSION,'5.0.0','<'))die('Texy! needs PHP version 5');define('TEXY','Version 2.0beta $Revision: 75 $');define('TEXY_DIR',dirname(__FILE__).'/');
...
function
factory($level,$mContent,$mMod1,$mMod2,$mMod3,$mMod4){$tx=$this->texy;$mod=new
TexyModifier;$mod->setProperties($mMod1,$mMod2,$mMod3,$mMod4);$user=NULL;if(is_callable(array($tx->handler,'heading')))$el=$tx->handler->heading($tx,$level,$mContent,$mod,$user);$el=new
...
FALSE;$el=TexyHtml::el($tag);if($aTags===Texy::ALL&&$isEmpty)$el->_empty=TRUE;if(!$isOpening){if(is_callable(array($tx->handler,'htmlTag')))$tx->handler->htmlTag($tx,$el,FALSE);return$tx->protect($el->endTag(),$el->getContentType());}if(is_array($aAttrs))$aAttrs=array_flip($aAttrs);else$aAttrs=NULL;preg_match_all('#([a-z0-9:-]+)\s*(?:=\s*(\'[^\']*\'|"[^"]*"|[^\'"\s]+))?()#is',$mAttr,$matches2,PREG_SET_ORDER);foreach($matches2
...
FALSE;if(isset($el->href)){$tx->summary['links'][]=$el->href;}}if(is_callable(array($tx->handler,'htmlTag')))$tx->handler->htmlTag($tx,$el,TRUE);return$tx->protect($el->startTag(),$el->getContentType());}} 
...
function
processBlock($parser,$matches){list(,$mURLs,$mImgMod1,$mImgMod2,$mImgMod3,$mImgMod4,$mLink,$mContent,$mMod1,$mMod2,$mMod3,$mMod4)=$matches;$tx=$this->texy;$user=$link=NULL;$image=$tx->imageModule->parse($mURLs,$mImgMod1,$mImgMod2,$mImgMod3,$mImgMod4);$mod=new
TexyModifier;$mod->setProperties($mMod1,$mMod2,$mMod3,$mMod4);if($mLink){if($mLink===':'){$link=new
...
function
parse($mURLs,$mMod1,$mMod2,$mMod3,$mMod4){$image=$this->getReference(trim($mURLs));if(!$image){$image=$this->parseContent($mURLs);$image->modifier=new
TexyModifier;}$image->modifier->setProperties($mMod1,$mMod2,$mMod3,$mMod4);return$image;}public
function
...
function
processLine($parser,$matches){list(,$mURLs,$mMod1,$mMod2,$mMod3,$mMod4,$mLink)=$matches;$tx=$this->texy;$user=$link=NULL;$image=$this->parse($mURLs,$mMod1,$mMod2,$mMod3,$mMod4);if($mLink){if($mLink===':'){$link=new
TexyLink;$link->URL=$image->linkedURL===NULL?$image->imageURL:$image->linkedURL;$link->type=TexyLink::AUTOIMAGE;$link->modifier=new
...
function
parse($dest,$mMod1,$mMod2,$mMod3,$label){$tx=$this->texy;$type=TexyLink::NORMAL;if(strlen($dest)>1&&$dest{0}==='['&&$dest{1}!=='*'){$type=TexyLink::REF;$dest=substr($dest,1,-1);$link=$this->getReference($dest);}elseif(strlen($dest)>1&&$dest{0}==='['&&$dest{1}==='*'){$type=TexyLink::IMAGE;$dest=trim(substr($dest,2,-2));$image=$tx->imageModule->getReference($dest);if($image){$link=new
...
TexyModifier;}$link->URL=str_replace('%s',urlencode(Texy::wash($label)),$link->URL);$link->modifier->setProperties($mMod1,$mMod2,$mMod3);$link->type=$type;return$link;}public
function
...
function
init(){$tx=$this->texy;$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\*)\*\*\*(?!\ |\*)(.+)'.TEXY_MODIFIER.'?(?<!\ |\*)\*\*\*(?!\*)()'.TEXY_LINK.'??()#U','phraseStrongEm');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\*)\*\*(?!\ |\*)(.+)'.TEXY_MODIFIER.'?(?<!\ |\*)\*\*(?!\*)'.TEXY_LINK.'??()#U','phraseStrong');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<![/:])\/\/(?!\ |\/)(.+)'.TEXY_MODIFIER.'?(?<!\ |\/)\/\/(?!\/)'.TEXY_LINK.'??()#U','phraseEm');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\*)\*(?!\ |\*)(.+)'.TEXY_MODIFIER.'?(?<!\ |\*)\*(?!\*)'.TEXY_LINK.'??()#U','phraseEmAlt');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\+)\+\+(?!\ |\+)(.+)'.TEXY_MODIFIER.'?(?<!\ |\+)\+\+(?!\+)()#U','phraseIns');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\-)\-\-(?!\ |\-)(.+)'.TEXY_MODIFIER.'?(?<!\ |\-)\-\-(?!\-)()#U','phraseDel');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\^)\^\^(?!\ |\^)(.+)'.TEXY_MODIFIER.'?(?<!\ |\^)\^\^(?!\^)()#U','phraseSup');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\_)\_\_(?!\ |\_)(.+)'.TEXY_MODIFIER.'?(?<!\ |\_)\_\_(?!\_)()#U','phraseSub');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\")\"(?!\ )([^\"]+)'.TEXY_MODIFIER.'?(?<!\ )\"(?!\")'.TEXY_LINK.'??()#U','phraseSpan');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\~)\~(?!\ )([^\~]+)'.TEXY_MODIFIER.'?(?<!\ )\~(?!\~)'.TEXY_LINK.'??()#U','phraseSpanAlt');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\~)\~\~(?!\ |\~)(.+)'.TEXY_MODIFIER.'?(?<!\ |\~)\~\~(?!\~)'.TEXY_LINK.'??()#U','phraseCite');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\>)\>\>(?!\ |\>)(.+)'.TEXY_MODIFIER.'?(?<!\ |\<)\<\<(?!\<)'.TEXY_LINK.'??()#U','phraseQuote');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!\")\"(?!\ )([^\"]+)'.TEXY_MODIFIER.'?(?<!\ )\"(?!\")\(\((.+)\)\)()#U','phraseAcronym');$tx->registerLinePattern(array($this,'processPhrase'),'#(?<!['.TEXY_CHAR.'])(['.TEXY_CHAR.']{2,})()()()\(\((.+)\)\)#Uu','phraseAcronymAlt');$tx->registerLinePattern(array($this,'processNoTexy'),'#(?<!\')\'\'(?!\ |\')([^'.TEXY_MARK.']*)(?<!\ |\')\'\'(?!\')()#U','phraseNoTexy');$tx->registerLinePattern(array($this,'processPhrase'),'#\`(\S[^'.TEXY_MARK.']*)'.TEXY_MODIFIER.'?(?<!\ )\`()#U','phraseCode');$tx->registerLinePattern(array($this,'processPhrase'),'#(['.TEXY_CHAR.'0-9@\#$%&.,_-]+)()()()(?=:\[)'.TEXY_LINK.'()#Uu','phraseQuickLink');$tx->registerBlockPattern(array($this,'processBlock'),'#^`=(none|code|kbd|samp|var|span)$#mUi','phraseCodeSwitch');}public
function
processPhrase($parser,$matches,$phrase){list($match,$mContent,$mMod1,$mMod2,$mMod3,$mLink)=$matches;$tx=$this->texy;$parser->again=$phrase!=='phraseCode'&&$phrase!=='phraseQuickLink';$mod=new
TexyModifier;$mod->setProperties($mMod1,$mMod2,$mMod3);$link=$user=NULL;$tag=isset($this->tags[$phrase])?$this->tags[$phrase]:NULL;if($tag==='a'){if($mLink==NULL){if(!$mMod1&&!$mMod2&&!$mMod3)return
FALSE;$tag='span';}elseif(empty($tx->allowed['phraseHasLink'])){return$mContent;}else{$link=$tx->linkModule->parse($mLink,$mMod1,$mMod2,$mMod3,$mContent);$tag=NULL;}}elseif($tag==='acronym'){$mod->title=trim(Texy::decode($mLink));}elseif($tag==='q'){$mod->cite=$tx->quoteModule->citeLink($mLink);}elseif($mLink!=NULL&&!empty($tx->allowed['phraseHasLink'])){$link=$tx->linkModule->parse($mLink,NULL,NULL,NULL,$mContent);}if(is_callable(array($tx->handler,'phrase'))){$el=$tx->handler->phrase($tx,$phrase,$mContent,$mod,$link,$user);if($el)return$el;}if($phrase==='phraseCode')$el=$tx->protect(Texy::encode($mContent),Texy::CONTENT_TEXTUAL);else$el=$mContent;if($phrase==='phraseStrongEm'){$el=TexyHtml::el($this->tags['phraseEm'])->setContent($el);$tag=$this->tags['phraseStrong'];}if($tag){$el=TexyHtml::el($tag)->setContent($el);$mod->decorate($tx,$el);}if($tag==='q')$el->cite=$mod->cite;if($link)$el=$tx->linkModule->factory($link)->setContent($el);if(is_callable(array($tx->handler,'phrase2')))$tx->handler->phrase2($tx,$el,$phrase,$user);return$el;}public
function
...
TexyModifier;$mod->setProperties($mMod1,$mMod2,$mMod3,$mMod4);$mod->decorate($tx,$el);$content='';$spaces='';do{if($mSpaces===':'){$mod->cite=$tx->quoteModule->citeLink($mContent);$content.="\n";}else{if($spaces==='')$spaces=max(1,strlen($mSpaces));$content.=$mContent."\n";}if(!$parser->receiveNext("#^>(?:|(\\ {1,$spaces}|:)(.*))()$#mA",$matches))break;list(,$mSpaces,$mContent)=$matches;}while(TRUE);$el->cite=$mod->cite;$el->parseBlock($tx,$content);$parser->children[]=$el;}public
function
...
function
init(){$pairs=array('#(?<![.\x{2026}])\.{3,4}(?![.\x{2026}])#mu'=>"\xe2\x80\xa6",'#(?<=[\d ])-(?=[\d ])#'=>"\xe2\x80\x93",'#,-#'=>",\xe2\x80\x93",'#(?<!\d)(\d{1,2}\.) (\d{1,2}\.) (\d\d)#'=>"\$1\xc2\xa0\$2\xc2\xa0\$3",'#(?<!\d)(\d{1,2}\.) (\d{1,2}\.)#'=>"\$1\xc2\xa0\$2",'#([\x{2013}\x{2014}]) #u'=>"\$1\xc2\xa0",'# --- #'=>" \xe2\x80\x94\xc2\xa0",'# -- #'=>" \xe2\x80\x93\xc2\xa0",'# -> #'=>" \xe2\x86\x92 ",'# <- #'=>" \xe2\x86\x90 ",'# <-> #'=>" \xe2\x86\x94 ",'#(\d+)( ?)x\\2(\d+)\\2x\\2(\d+)#'=>"\$1\xc3\x97\$3\xc3\x97\$4",'#(\d+)( ?)x\\2(\d+)#'=>"\$1\xc3\x97\$3",'#(?<=\d)x(?= |,|.|$)#m'=>"\xc3\x97",'#(\S ?)\(TM\)#i'=>"\$1\xe2\x84\xa2",'#(\S ?)\(R\)#i'=>"\$1\xc2\xae",'#\(C\)( ?\S)#i'=>"\xc2\xa9\$1",'#\(EUR\)#'=>"\xe2\x82\xac",'#(\d{1,3}) (\d{3}) (\d{3}) (\d{3})#'=>"\$1\xc2\xa0\$2\xc2\xa0\$3\xc2\xa0\$4",'#(\d{1,3}) (\d{3}) (\d{3})#'=>"\$1\xc2\xa0\$2\xc2\xa0\$3",'#(\d{1,3}) (\d{3})#'=>"\$1\xc2\xa0\$2",'#(?<=.{50}) +(?=[\x17-\x1F]*\S{1,6}[\x17-\x1F]*$)#u'=>"\xc2\xa0",'#(?<=^| |\.|,|-|\+|\x16)([\x17-\x1F]*\d+[\x17-\x1F]*) ([\x17-\x1F]*['.TEXY_CHAR.'\x{b0}-\x{be}\x{2020}-\x{214f}])#mu'=>"\$1\xc2\xa0\$2",'#(?<=^|[^0-9'.TEXY_CHAR.'])([\x17-\x1F]*[ksvzouiKSVZOUIA][\x17-\x1F]*) ([\x17-\x1F]*[0-9'.TEXY_CHAR.'])#mu'=>"\$1\xc2\xa0\$2",);if(isset(self::$locales[$this->locale])){$info=self::$locales[$this->locale];$pairs['#(?<!"|\w)"(?!\ |")(.+)(?<!\ |")"(?!")()#U']=$info['doubleQuotes'][0].'$1'.$info['doubleQuotes'][1];$pairs['#(?<!\'|\w)\'(?!\ |\')(.+)(?<!\ |\')\'(?!\')()#Uu']=$info['singleQuotes'][0].'$1'.$info['singleQuotes'][1];}$this->pattern=array_keys($pairs);$this->replace=array_values($pairs);}public
function

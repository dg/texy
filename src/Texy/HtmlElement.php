...
use function array_splice, count, http_build_query, implode, is_array, is_object, is_string, str_replace;
...
class HtmlElement implements \ArrayAccess, /* Countable, */ \IteratorAggregate
{
	...
	final public function setName(?string $name, ?bool $empty = null): static
	{
		...
		$this->isEmpty = $empty === null
			? isset(Output\Html\Schema::voidElements()[$name ?? ''])
			: (bool) $empty;
		return $this;
	}
	...
	{
		$inlineType = Output\Html\Schema::inlineElements()[$this->name ?? ''] ?? null;

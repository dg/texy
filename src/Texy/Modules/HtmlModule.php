...
final class HtmlModule extends Texy\Module
{
	...
	public function __construct(
		...
		$texy->htmlOutput->registerHandler($this->solveTag(...));
		$texy->htmlOutput->registerHandler($this->solveComment(...));
	}
	...
	public function solveTag(HtmlTagNode $node, Html\Renderer $generator): string
	{
		...
		$inlineType = Html\Schema::inlineElements()[$tagName] ?? null;
		if ($inlineType === null) {
	...
	public function solveComment(HtmlCommentNode $node, Html\Renderer $generator): string
	{
		if (!$this->passComment) {
			return '';
		}
